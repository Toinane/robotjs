...
#define MMBitmapPointInBounds(image, p) ((p).x < (size_t)(image)->width && \
                                         (p).y < (size_t)(image)->height)
...
#define MMRGBColorRefAtPoint(image, x, y) \
	(MMRGBColor *)(assert((size_t)(x) < (size_t)(image)->width && (size_t)(y) < (size_t)(image)->height), \
